  ...
  router.post('/quote', async (req: Request, res: Response) => {
    ...
    try {
      ...
      res.status(200).json({
        success: true,
        data: {
          price: inrPrice, // Rounded to 2 decimal places
          min: 1,
        ...
        for (let i = 0; i < transactions?.length; i++) {
          ...
          if (inMessage) {
            ...
            responseData.push({
              ...
              outgoingAddresses:transaction.in_msg.destination &&   Address.parse(transaction.in_msg.destination).toString(), // Array of outgoing wallet addresses
