...
import { getSwapOptions, createTokens, createAmountInWei, getRouter, approveToken, performSwap } from "../services/swapService.js";
...
router.post("/swap", async (req, res) => {
  ...
  try {
    const {
      tokenInput: { address: tokenInputAddress, decimal: tokenInputDecimal, symbol: tokenInputSymbol, name: tokenInputName, amount: tokenInputAmount },
      tokenOutput: { address: tokenOutputAddress, decimal: tokenOutputDecimal, symbol: tokenOutputSymbol, name: tokenOutputName }
    } = req.body;

    const { inputToken, outputToken } = createTokens(config.chainId, tokenInputAddress, tokenInputDecimal, tokenInputSymbol, tokenInputName, tokenOutputAddress, tokenOutputDecimal, tokenOutputSymbol, tokenOutputName);
    const inputAmountInWei = createAmountInWei(tokenInputAmount, tokenInputDecimal);
    ...
    await approveToken(wallet, tokenInputAddress, route.methodParameters.to, inputAmountInWei);
